...
traits <- read.csv("Analysis_SaltyHel17/Data/trimmedSALTYHEL17_traits.csv", header = TRUE)
...
branched_t <- subset(traits, Branching > 0)
unbranched_t <- subset(traits, Branching == 0)
...
ggplot(ind, aes(x=PC1, y=PC4, color=Oil)) +
  geom_point(shape=1)    # Use hollow circles

# write csv if needed
write.csv(ind, "PCA_Unbranched_saltyHel17.csv")
