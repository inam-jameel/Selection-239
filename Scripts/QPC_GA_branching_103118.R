...
branched_t <- subset(traits, Branching > 0, select=Lineage:T_branches)
unbranched_t <- subset(traits, Branching == 0, select=Lineage:T_branches)
All_t <- subset(traits, select=Lineage:T_branches)

### Remove individuals in SNP dataset that were removed from the branched 
### and unbranched trait dataset and make specific SNP datasets for each trait dataset 
...
traitdata <- read.csv("Analysis_GA/Data/239_trait_means_102618.csv", header = FALSE, stringsAsFactors = FALSE)
...
mydfs <- apply(
  ...
  #branched_t
  #All_t
  [,-c(1:4)], 2, function(x){calcQpc(
    myZ = x, myU = eigF$vectors, myLambdas = eigF$values, myPCcutoff = 0.5
  )})
...
# specify the dfs.., 1 : n  
allpvals <- sapply(1:19, function(x){mydfs[[x]]$pvals})
...
axis(2, at=(0:18)/18, labels = traitlabels, las=2)
...
#############################################################################################################################
#############################################################################################################################
pnames <- as.character(unbranched_t$Plant)
...
mydf <- data.frame(unbranched_t[-nrow(unbranched_t),], eigF$vectors, stringsAsFactors = F)
